...
public class DenoiseUI extends JFrame implements Runnable {
	/**
		...
		synchronized (this) {
			...
			for (int i = 0; i < files.length; i++) {
				if (state == RUN && fileCount < files.length) {
					...
					DenoiseImp denoise = new DenoiseImp(files[fileCount], out);
					denoise.denoise();
	...
	class StartDenoiseListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (files == null) {
				JOptionPane.showMessageDialog(DenoiseUI.this, "请选择目录！", "错误",
			...
			denoiseDir = files[0].getParent() + "Denoise" + File.separator
					+ "Denoise";
			new File(denoiseDir).mkdirs();
			denoiseDetailTa.setText("");
			denoiseThread1 = new Thread(DenoiseUI.this);
	...
	class SelectDirListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			int state = jc.showOpenDialog(DenoiseUI.this);// ((Component)e.getSource()).getParent()
	...
	class OpenDirListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (denoiseDir == null) {
				JOptionPane.showMessageDialog(DenoiseUI.this,
