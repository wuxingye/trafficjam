...
public class TimeDenoiseUI extends JFrame implements Runnable {
	/**
	...
	private JLabel gpsDirLabel, gpsInfoLabel, gpsTimeIntervalLabel,
			denoiseDetailLabel, resultLabel;
	private JTextField gpsDirTf, gpsInfoTf, gpsTimeIntervalTF;
	...
	public TimeDenoiseUI(String title) {
		super(title);
		contentPane = getContentPane();
		contentPane.setLayout(gridbag);
		c.fill = GridBagConstraints.HORIZONTAL;

		gpsDirLabel = new JLabel("待过滤GPS所在目录：");
		...
		gpsInfoLabel = new JLabel("待过滤GPS目录信息：");
		gpsInfoTf = new JTextField(50);
		gpsInfoTf.setEditable(false);
		gpsTimeIntervalLabel = new JLabel("待过滤GPS时间间隔:");
		gpsTimeIntervalTF = new JTextField(10);
		gpsTimeIntervalTF.setEditable(true);
		denoisePane = new JPanel();
		denoisePane.setLayout(new FlowLayout());
		startDenoise = new JButton("开始过滤");
		stopDenoise = new JButton("暂停过滤");
		exitDenoise = new JButton("停止过滤");
		...
		addComponent(gpsTimeIntervalLabel, 2, 0, 2, 1);
		addComponent(gpsTimeIntervalTF, 2, 1, 2, 1);

		addComponent(denoisePane, 3, 0, 3, 1);

		denoiseDetailLabel = new JLabel("过滤详情：");
		addComponent(denoiseDetailLabel, 4, 0, 1, 1);

		denoiseDetailTa = new JTextArea(10, 30);
		scrollPane = new JScrollPane(denoiseDetailTa,
				JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		addComponent(scrollPane, 5, 0, 3, 1);

		resultLabel = new JLabel(" ");
		openDir = new JButton("打开目录");
		openDir.addActionListener(new OpenDirListener());
		addComponent(resultLabel, 6, 0, 3, 1);
		addComponent(openDir, 7, 0, 3, 1);
		...
		synchronized (this) {
			...
			for (int i = 0; i < files.length; i++) {
				if (state == RUN && fileCount < files.length) {
					...
					TimeDenoiseImp denoise = new TimeDenoiseImp(
							files[fileCount], out,
							Long.parseLong(gpsTimeIntervalTF.getText()));// TODO
					denoise.denoise();
	...
	class StartDenoiseListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			try {
				Long.parseLong(gpsTimeIntervalTF.getText());
			} catch (Exception e2) {
				JOptionPane.showMessageDialog(TimeDenoiseUI.this, "时间间隔输入有误！", "错误",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			if (files == null) {
				JOptionPane.showMessageDialog(TimeDenoiseUI.this, "请选择目录！", "错误",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			startDenoise.setEnabled(false);
			...
			denoiseDir = files[0].getParent() + "TimeDenoise" + File.separator
					+ "Denoise";
			new File(denoiseDir).mkdirs();
			denoiseDetailTa.setText("");
			denoiseThread1 = new Thread(TimeDenoiseUI.this);
	...
	class SelectDirListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			int state = jc.showOpenDialog(TimeDenoiseUI.this);// ((Component)e.getSource()).getParent()
	...
	class OpenDirListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (denoiseDir == null) {
				JOptionPane.showMessageDialog(TimeDenoiseUI.this,
						"未找到目录，请执行过滤后打开目录！", "错误", JOptionPane.ERROR_MESSAGE);
				return;
			}
			try {
				Runtime.getRuntime()
